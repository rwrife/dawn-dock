...
**Baseline:** v0.1
**Evidence status:** normative behavior specification with partial host software-test coverage in `firmware/host`; occurrence lifecycle, committed-snapshot UTC occurrence evaluation (including active snooze/timeout progression and reboot recovery through the persisted evaluator path), confirmed alarm disable through the atomic store (alarmDisabled dismiss journal before the stop edge, then the definition change as a new revision), invalid-interval reconciliation of crossed committed occurrences (newest rings within the frozen 10-minute grace, older crossed occurrences journal invalidTime misses, the whole batch commits through the runtime CAS before any alert), race-safe dual-slot schedule/runtime-journal storage, pure weekly recurrence against supplied versioned timezone transitions, deterministic recurrence-to-committed-UTC materialization, a host-tested pinned TZif-to-rules adapter reproducing the committed gap/fold instants from sha256-pinned IANA 2026c records, and a host-tested runtime coordinator composing invalid-time freeze, boot-gated reboot recovery, invalid-interval reconciliation, and committed evaluation into one deterministic tick order are covered. On-device rule-data embedding/updates, on-device integration of the runtime coordinator (RTC validity, boot identification, tick scheduling), target execution, and bench evidence remain open.
