...
drafts, schedule mirror, versioned backup, reviewable schedule diff, and the
schedule preview/apply exchange framing core) is software-only evidence: the
code is not wired to any screen, filesystem, or transport, its tests run on the
host Dart VM, and none of the open items above change because it exists.
