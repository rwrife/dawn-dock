...
| ALARM-01 | State transitions match `docs/alarm-semantics.md` | Host state-machine fixture; target interaction tests | Host software tests cover due/disabled admission, duplicate suppression, snooze/re-ring, dismiss, timeout, missed, reboot recovery, local-weekday recurrence, calendar rollover, occurrence-journal/schedule-update race preservation, and committed-snapshot UTC evaluation. The persisted evaluator now advances active snooze expiry, timeout stop edges, and once-per-boot recovery before exposing alert effects. Recurrence-to-snapshot integration, disable-after-terminal, invalid-time reconciliation, target, and bench paths remain open |
