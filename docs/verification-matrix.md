...
| ALARM-01 | State transitions match `docs/alarm-semantics.md` | Host state-machine fixture; target interaction tests | Host software tests cover due/disabled admission, duplicate suppression, snooze/re-ring, dismiss, timeout, missed, reboot recovery, local-weekday recurrence, calendar rollover, occurrence-journal/schedule-update race preservation, and committed-snapshot UTC evaluation. The evaluator durably orders catch-up outcomes before exposing alerts and suppresses effects on schedule races, identical concurrent admission, and write failure. Recurrence-to-snapshot integration, active lifecycle orchestration, disable-after-terminal, invalid-time reconciliation, target, and bench paths remain open |
