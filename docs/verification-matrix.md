...
| ALARM-01 | State transitions match `docs/alarm-semantics.md` | Host state-machine fixture; target interaction tests | Host software tests cover due/disabled admission, duplicate suppression, snooze/re-ring, dismiss, timeout, missed, reboot recovery, local-weekday recurrence, calendar rollover, and occurrence-journal/schedule-update race preservation. The committed-schedule evaluator, disable-after-terminal, invalid-time reconciliation, target, and bench paths remain open |
