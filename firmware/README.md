...
CRC32 detects accidental corruption; it is not authentication. `components/nvs_storage` now binds the slots to independent ESP-IDF NVS blobs with partition-wide exclusive transactions, bounded tri-state reads, explicit commits, and a reboot-only fault latch on uncertain writes. `app_main` only initializes/inspects storage; it never provisions an alarm or auto-erases. See [`docs/nvs-storage.md`](../docs/nvs-storage.md) for the 256 KiB partition budget, changed factory offset/upgrade warning, API basis, host fake coverage, and evidence limits. These host fault-injection tests do not prove flash endurance, brownout behavior, or physical power-cycle retention.
...
- ESP-IDF NVS target execution, brownout/flash fault behavior, partition-upgrade recovery, and 100-cycle physical persistence evidence (the adapter is now host-tested against fake APIs);
